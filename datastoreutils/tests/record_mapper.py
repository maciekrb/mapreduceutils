...
"""
Implements Unit tests for transformations occurring inside of 
a mapreduce pipeline

Transformation cases to implement:
  - Date conversion 
  - Search and replace
  - Round / Ceil / Floor
  - Resolve Higher Controlled domain term
"""

def computed_property_dummy_func(record, attr_name):
  """ Dummy function to test a property_list computed attribute """
  return getattr(record, attr_name).timetuple().tm_yday
...
class DummyReferenceModel(db.Expando):
  pass

class DummyDatastoreModel_db(db.Expando):
  record_entry = db.ReferenceProperty(reference_class=DummyReferenceModel, collection_name="records")
  tag_keys = db.ListProperty(db.Key)
  created_time = db.DateTimeProperty()
  data_quality = db.IntegerProperty(default=1)
  schema = db.ReferenceProperty(reference_class=DummyReferenceModel, collection_name="schema_records")
  schema_name = db.StringProperty()


class TestDatastoreMap(testutil.HandlerTestBase):
  ...
  def test_map_pipeline_csv_conversion(self):
    ...
    for r in test_data:
      DummyDatastoreModel_db(**r).put()

    output_data = _run_pipeline(
      self.taskqueue, 
      __name__ + ".DummyDatastoreModel_db",
      __name__ + ".record_map",
      params={
        "property_map": [{
  ...
  def test_map_pipeline_property_map(self):
    ...
    DummyDatastoreModel_db(**record_data).put()
  ...
  def test_map_pipeline_reference_property_mapping(self):
    ...
    record_entry = DummyReferenceModel(context="some_context").put()
    schema  = DummyReferenceModel(name="cobertura_vegetal").put()
    tag_ABC = DummyReferenceModel(name="ABC").put()
    tag_BCD = DummyReferenceModel(name="BCD").put()
  ...
  def test_map_pipeline_eval_name(self):
    """ Property list can contain qualified names to evaluate functions """
      ...
      params={
        "property_map": [{
          ...
          "property_list": [
            ...
            "nivel_1",
            "data_quality",
            ("my_computed_attr", # Name of attribute
             __name__ + ".computed_property_dummy_func", # evaluator func
             { "attr_name": "created_time" } # evaluator func args
            )
          ]
        }]
      }
    ...
    # Assert number of rows, and format of one of them. Order is not predictable
    self.assertEquals(1, len(output_data))
    rec = '2010-08-12 18:23:20,cobertura_vegetal,Bosques y Áreas Seminaturales,1,224\r\n'
    self.assertEquals(rec, output_data[0])
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(TestDatastoreMap))
    return suite
